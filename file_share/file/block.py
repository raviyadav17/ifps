...
class Blockchain:
    ...
    def replace_chain(self):
        ...
        if longest_chain:
            ...
            return True
        return False
# Create a node and test the blockchain
node = Blockchain()


node.add_file('Alice', 'Bob', 'fgqgfwqiug')
node.create_block(node.proof_of_work(node.get_previous_block()['proof']), node.hash(node.get_previous_block()))

node.add_file('Bob', 'Charlie', 'hqugfuqegqeiug')
node.create_block(node.proof_of_work(node.get_previous_block()['proof']), node.hash(node.get_previous_block()))
